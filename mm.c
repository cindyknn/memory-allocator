...
#define MIN_BLK_SIZE (2 * DSIZE)    /* Minimum block size (bytes): 1 DSIZE for header and footer, 1 DSIZE for prev and next pointers. */
...
/* Define a struct containing pointers to the previous and next free blocks in explicit free list. */
...
static void ins_flist(void *bp, size_t size); /* Function to insert a block into the segregated list corresponding to its size in explicit free list. */
static void rem_flist(void *bp); /* Function to remove a block from the segregated list in explicit free list */
static fblock *size_bin(size_t size); /* Find the size bin corresponding to given size. */
...
mm_malloc(size_t size)
{
    ...
    /// Simple segregated storage v1: Round off smaller size bins (2 to power of 4 to 9) for efficiency.
    if (size <= 16) {
	size = 16;
    } else if (size <= 32) {
	size = 32;
	...
	asize = ALIGNMENT * ((size + DSIZE + (ALIGNMENT - 1)) / ALIGNMENT);
...
mm_realloc(void *ptr, size_t size)
{
    ...
    /* Check if the next block is free, if yes then maybe can coalesce with current block to make enough space for realloc size. */
    size_t csize = 0;
    newptr = ptr;
	...
	if (prev_alloc && !next_alloc && (oldsize + GET_SIZE(HDRP(NEXT_BLKP(ptr))) >= size)) {
	    rem_flist(NEXT_BLKP(ptr));
	    csize = oldsize + GET_SIZE(HDRP(NEXT_BLKP(ptr))) + DSIZE;
	} else if (!prev_alloc && !next_alloc && (oldsize + GET_SIZE(HDRP(PREV_BLKP(ptr))) + GET_SIZE(HDRP(NEXT_BLKP(ptr))) >= size)) {
	    rem_flist(PREV_BLKP(ptr));
	    rem_flist(NEXT_BLKP(ptr));
	    csize = oldsize + GET_SIZE(HDRP(PREV_BLKP(ptr))) + GET_SIZE(HDRP(NEXT_BLKP(ptr))) + DSIZE;
	...
	void *extra;
	size_t asize = ALIGNMENT * ((size + DSIZE + (ALIGNMENT - 1)) / ALIGNMENT);
	if ((extra = mem_sbrk(asize - oldsize - DSIZE)) == (void *)-1)
	    return (NULL);

	/* Initialize free block header/footer and the epilogue header. */
	PUT(HDRP(ptr), PACK((asize), 1));    /* Update header of current block with new size. */
	PUT(FTRP(ptr), PACK((asize), 1));    /* Update new footer of current block with new size. */
...
coalesce(void *bp)
{
    ...
    /* Remove the prev/next free block that bp coalesces with from free list. Insert new coalesced free block into free list. */
...
find_fit(size_t asize)
{
	...
	while (bin < (flist + (NUM_BINS-1))) {
	    /* If can't find fit in this bin, check next bin. */
	    bin += 1;
...
{
    ///FIFO queue EFL for now: insert to end, remove from front
...
/*
* Remove a free block from explicit free list.
*/
static void 
rem_flist(void *bp)
{
...
checkblock(void *bp)
{
    ...
    /* Check if header matches footer, including if block size and allocation bit are correct. */
    if (GET(HDRP(bp)) != GET(FTRP(bp)))
	printf("Error: header does not match footer.\n");

    /* If the block is free, check the previous and next pointers and check for coalescing. */
    if (!GET_ALLOC(HDRP(bp))) {
	/* Check if the previous and next pointers point to valid free blocks. */
	fblock *prev = ((fblock *)bp)->prev;
	fblock *next = ((fblock *)bp)->next;
	if (prev == NULL || GET_ALLOC(HDRP(prev)))
	    printf("Error: previous pointer does not point to a valid free block.\n");
	if (next == NULL || GET_ALLOC(HDRP(next)))
	    printf("Error: pointer does not point to a valid free block.\n");

	/* Check if there is any coalescing possible but has not yet been done. */
	if (!GET_ALLOC(HDRP(NEXT_BLKP(bp))) || !GET_ALLOC(HDRP(PREV_BLKP(bp)))) {
	    printf("Error: can coalesce with an adjacent free block but has not yet done so.\n");
	}
    }
...
///EFL: Free list check: correct size bin

///DONE: 8-byte aligned, size & alloc bit (check if footer match)
///DONE- EFL: free list blocks- check p&n pointers to valid free blocks? any coalescing can do but havent
...
checkheap(bool verbose)
{
    ...
    /* Check all segregated lists in the explicit free list: if all the blocks are in the correct size bin. */
    size_t bin_sizes[NUM_BINS] = {0, 32, 128, 512, 2048, 16384};
    size_t bin_index = 0; /* Keep track of the index of the current bin. */
    for (fblock *currlist = flist; currlist < flist + NUM_BINS; currlist += 1) {
	fblock *currblock = currlist->next;
	while (currblock != currlist) {
	    size_t size = GET_SIZE(HDRP(currblock));
	    if (bin_index == NUM_BINS - 1) {
		if (size <= bin_sizes[bin_index])
			printf("Error: free block is not in correct size bin.\n");
	    } else {
		if (size <= bin_sizes[bin_index] || size > bin_sizes[bin_index + 1])
			printf("Error: free block is not in correct size bin.\n");
	    }
