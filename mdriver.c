...
typedef struct {
	unsigned sugg_heapsize; /* suggested heap size (unused) */
	unsigned num_ids;	/* number of alloc/realloc ids */
	unsigned num_ops;	/* number of distinct requests */
	unsigned weight;	/* weight for this trace (unused) */
	traceop_t *ops;		/* array of requests */
	char **blocks; /* array of ptrs returned by malloc/realloc... */
	size_t
	    *block_sizes; /* ... and a corresponding array of payload sizes */
} trace_t;
...
main(int argc, char **argv)
{
	...
	double secs, ops, util, avg_mm_util, avg_mm_throughput, p1, p2,
	    perfindex;
	int numcorrect;
	 ...
	 */
	while ((c = getopt(argc, argv, "gf:t:vVh")) != EOF) {
		switch (c) {
		case 'g': /* Generate summary info for the autograder */
			autograder = 1;
			break;
		case 'f': /* Use one specific trace file only (relative to curr
			     dir) */
			num_tracefiles = 1;
			if ((tracefiles = realloc(tracefiles,
				 2 * sizeof(char *))) == NULL)
		...
		case 't': /* Directory where the traces are located */
			if (num_tracefiles ==
			    1) /* ignore if -f already encountered */
				break;
	...
	if (tracefiles == NULL) {
		...
		num_tracefiles = sizeof(default_tracefiles) / sizeof(char *) -
		    1;
	...
	if (errors == 0) {
		...
		printf(
		    "Perf index = %.0f/%.0f (util) + %.0f/%.0f (thru) = %.0f/100\n",
		    p1 * 100, UTIL_WEIGHT * 100, p2 * 100,
		    (1.0 - UTIL_WEIGHT) * 100, perfindex);
 ...
 */
static int
add_range(range_t **ranges, char *lo, int size, int tracenum, int opnum)
{
	char *hi = lo + size - 1;
	range_t *p;
	char msg[MAXLINE];
	...
	if ((lo < (char *)mem_heap_lo()) || (lo > (char *)mem_heap_hi()) ||
		...
		sprintf(msg, "Payload (%p:%p) lies outside heap (%p:%p)", lo,
 ...
 *
 * XXX alc: It's unclear what the variable "size" was used for.
 */
static void
remove_range(range_t **ranges, char *lo)
{
	range_t *p;
 ...
 */
static trace_t *
read_trace(char *tracedir, char *filename)
{
	FILE *tracefile;
	trace_t *trace;
	char type[MAXLINE];
	char path[896]; // XXX alc: This can't be MAXLINE.
	unsigned index, size;
	...
	fscanf(tracefile, "%u", &(trace->sugg_heapsize)); /* not used */
	fscanf(tracefile, "%u", &(trace->num_ids));
	fscanf(tracefile, "%u", &(trace->num_ops));
	fscanf(tracefile, "%u", &(trace->weight)); /* not used */

	/* We'll store each request line in the trace in this array */
	if ((trace->ops = (traceop_t *)malloc(
		 trace->num_ops * sizeof(traceop_t))) == NULL)
		unix_error("malloc 2 failed in read_trace");

	/* We'll keep an array of pointers to the allocated blocks here... */
	if ((trace->blocks = (char **)malloc(
		 trace->num_ids * sizeof(char *))) == NULL)
		unix_error("malloc 3 failed in read_trace");

	/* ... along with the corresponding byte sizes of each block */
	if ((trace->block_sizes = (size_t *)malloc(
		 trace->num_ids * sizeof(size_t))) == NULL)
 ...
 */
static int
eval_mm_valid(trace_t *trace, int tracenum, range_t **ranges)
{
	unsigned i, j;
	int index;
	unsigned size;
	...
	for (i = 0; i < trace->num_ops; i++) {
		...
		case ALLOC: /* mm_malloc */
			...
			/*
			 * Test the range of the new block for correctness and
			 * add it to the range list if OK. The block must be  be
			 * aligned properly, and must not overlap any currently
			 * allocated block.
			 */
			...
			/* ADDED: cgw
			 * fill range with low byte of index.  This will be used
			 * later if we realloc the block and wish to make sure
			 * that the old data was copied to the new block
			 */
		...
		case REALLOC: /* mm_realloc */
			...
			/* Check new block for correctness and add it to range
			 * list */
			if (add_range(ranges, newp, size, tracenum, i) == 0)
				return 0;

			/* ADDED: cgw
			 * Make sure that the new block contains the data from
			 * the old block and then fill in the new block with the
			 * low order byte of the new index
			 */
		...
		case FREE: /* mm_free */
			...
			/* Remove region from list and call student's free
			 * function */
 ...
 */
static double
eval_mm_util(trace_t *trace, int tracenum, range_t **ranges)
{
	unsigned i;
	int index;
	unsigned size, newsize, oldsize;
 ...
 */
static void
printresults(int n, stats_t *stats)
{
	int i;
	double secs = 0;
	double ops = 0;
	...
	/* All the space before the last number on each line is added by
	 * Zheng Cai, for better formatting */
	printf("%5s%7s %5s%8s%10s %6s\n", "trace", " valid", "util", "ops",
	    "secs", "Kops");
	for (i = 0; i < n; i++) {
		if (stats[i].valid) {
	...
	if (errors == 0) {
		printf("%12s%5.0f%%%8.0f%10.6f %6.0f\n", "Total       ",
		    (util / n) * 100.0, ops, secs, (ops / 1e3) / secs);
	} else {
 ...
 */
void
malloc_error(int tracenum, int opnum, char *msg)
{
	errors++;
	printf("ERROR [trace %d, line %d]: %s\n", tracenum, LINENUM(opnum),
	    msg);
}
...
usage(void)
{
	fprintf(stderr, "Usage: mdriver [-ghvV] [-f <file>] [-t <dir>]\n");
