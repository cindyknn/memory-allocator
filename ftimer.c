 ...
 */
double
ftimer_itimer(ftimer_test_funct f, void *argp, int n)
{
	double start, tmeas;
	int i;
 ...
 */
double
ftimer_gettod(ftimer_test_funct f, void *argp, int n)
{
	int i;
	...
	diff = 1E3 * (etv.tv_sec - stv.tv_sec) +
	    1E-3 * (etv.tv_usec - stv.tv_usec);
	diff /= n;
	return (1E-3 * diff);
}
